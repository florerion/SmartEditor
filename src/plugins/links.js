...
export const linkActions = [
  {
    ...
    run: (api) => {
      ...
      const text = hasSelection ? sel.text : 'link text';
      // eslint-disable-next-line no-alert
      const url = prompt('Enter URL:', 'https://');
      if (!url) return;

      api.replaceSelection(`[${text}](${url})`);

      if (!hasSelection) {
        const labelFrom = sel.from + 1;
        api.setSelection(labelFrom, labelFrom + text.length);
      }
    },
  },
  {
    ...
    run: (api) => {
      ...
      if (alt.trim()) {
        api.insertText(`![${alt}](${url})`, sel.to);
        return;
      }

      const placeholder = 'alt text';
      const markdown = `![${placeholder}](${url})`;
      api.insertText(markdown, sel.to);
      const altFrom = sel.to + 2;
      api.setSelection(altFrom, altFrom + placeholder.length);
    },
  },
];
